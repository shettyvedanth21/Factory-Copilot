...
class DeviceSimulator:
    ...
    def _publish_telemetry(self) -> None:
        ...
        if not self._telemetry_generator:
            return
        ...
        if self._config.ingest_mode == "http":
            self._publish_http(payload)
        else:
    ...
    def _publish_mqtt(self, payload: dict) -> None:
        ...
        if success:
            self._handle_publish_success(payload, "MQTT")
        else:
            logger.warning(
                "Failed to publish telemetry via MQTT",
                extra={"device_id": self._config.device_id}
            )

    def _publish_http(self, payload: dict) -> None:
        """Publish telemetry via HTTP POST."""
        import requests
        try:
            ...
            self._handle_publish_success(payload, "HTTP")
        except Exception as e:
            logger.warning(
                f"Failed to publish telemetry via HTTP: {e}",
                extra={"device_id": self._config.device_id}
            )

    def _handle_publish_success(self, payload: dict, method: str) -> None:
        """Handle successful telemetry publication."""
        self._message_count += 1
        logger.info(
            f"Telemetry published via {method}",
            extra={
