...
def parse_arguments() -> SimulatorConfig:
    ...
    env_ingest_mode = os.getenv("INGEST_MODE", "mqtt")
    env_ingest_url = os.getenv("INGEST_URL", "http://localhost:8081/api/v1/data/ingest")
    ...
    parser.add_argument(
        "--ingest-mode",
        type=str,
        default=env_ingest_mode,
        choices=["mqtt", "http"],
        help="Ingestion mode: mqtt or http (env: INGEST_MODE)"
    )

    parser.add_argument(
        "--ingest-url",
        type=str,
        default=env_ingest_url,
        help="HTTP ingestion URL (env: INGEST_URL)"
    )
