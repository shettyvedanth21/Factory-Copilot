...
def create_router() -> APIRouter:
    ...
    async def ingest_telemetry(
        ...
        try:
            # Re-use the existing MQTT processing logic
            await telemetry_service.process_telemetry_message(
                raw_payload=payload
            )
            
            return ApiResponse(
                success=True,
                data={"message": "Telemetry received and queued for processing"},
                timestamp=datetime.utcnow().isoformat(),
            )
        except Exception as e:
            logger.exception("Failed to ingest telemetry via HTTP")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail={
                    "success": False,
